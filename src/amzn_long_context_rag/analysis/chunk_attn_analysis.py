...
def _chunk_bounds_by_retriever(
    retriever,
    context_str: str,
) -> list[tuple[int, int, str]]:
    """
    Return a list with one entry per *retriever* chunk, each entry being
    `(start_char, end_char, chunk_text)` where the char-indices are **relative
    to the `context_str` alone**.
    """
    chunks = retriever._split_text(context_str)           # ⇐ 2 048 token rule
    bounds: list[tuple[int, int, str]] = []
    cursor = 0
    for chk in chunks:
        start = cursor
        end   = start + len(chk)
        bounds.append((start, end, chk))
        cursor = end
    return bounds


def _chunk_bounds_from_list(
    chunk_list: list[str],
    *,
    join_with: str = "\n",
) -> list[tuple[int, int, str]]:
    """
    Build (start_char, end_char, chunk_text) tuples **without re-tokenising**,
    assuming `context_str` was constructed with
    `join_with.join(chunk_list)`.
    """
    bounds = []
    cursor = 0
    for i, chk in enumerate(chunk_list):
        ...
        bounds.append((start, end, chk))
        cursor = end + (len(join_with) if i < len(chunk_list) - 1 else 0)
    return bounds
...
def _topk_attended_chunks_retriever(
    *,
    model,
    tokenizer,
    retriever,
    ...
    context_str: str,
    chunk_top_k: int,
    original_chunks: list[str] | None = None,   # ← NEW
) -> list[str]:
    """
    If `original_chunks` is given we aggregate attention **exactly over those
    chunks**; otherwise we fall back to retriever._split_text(context_str).
    """
    # 1) prompt → ids / offsets / str
    input_ids, offsets, prompt_str = _prepare_prompt(tokenizer, messages)

    # 2) attention for each model token
    tok_scores = _token_attention_scores(model=model, input_ids=input_ids)

    # 3) locate context in the prompt
    ctx_start = prompt_str.find(context_str)
    if ctx_start < 0:
        raise RuntimeError("Failed to locate context in the prompt string.")

    # 4) choose chunk boundaries
    if original_chunks is None:
        chk_bounds = _chunk_bounds_by_retriever(retriever, context_str)
    else:
        chk_bounds = _chunk_bounds_from_list(original_chunks)

    chk_ends = [end for _, end, _ in chk_bounds]

    # 5) accumulate scores per chunk
    from collections import defaultdict
    chk_scores = defaultdict(float)
    ...
    for score, (beg, _end) in zip(tok_scores.tolist(), offsets, strict=False):
        ...
        idx = bisect_right(chk_ends, rel_pos)
        if idx < len(chk_bounds):
            _, _, chk_txt = chk_bounds[idx]
            chk_scores[chk_txt] += score

    ranked = sorted(chk_scores.items(), key=lambda kv: kv[1], reverse=True)
    if chunk_top_k < 0:
        return [t for t, _ in ranked]
    return [t for t, _ in ranked[: min(chunk_top_k, len(ranked))]]
...
# Core routine
# ---------------------------------------------------------------------------
def chunk_attn_analysis(config_path: str, dotlist: list[str] | None) -> None:  # noqa: C901
    """Run chunk-attention analysis on a dataset, with and without RAG."""
    ...
    generation_cfg: Mapping[str, Any] = cfg["generation_config"]
    ...
    # The number of chunks for the attention intersection
    chunk_size: int = retrieval_cfg.get("max_tokens", 2048)
    chunk_top_k: int = cfg.get("chunk_top_k", 10)
    ...
    if context_max_tokens:
        out_path = out_subdir / f"chunk_{config_name}_maximum_input_length={maximum_input_length}_chunk_top_k={chunk_top_k}_context_max_tokens={context_max_tokens}.jsonl"
    else:
        out_path = out_subdir / f"chunk_{config_name}_maximum_input_length={maximum_input_length}_chunk_top_k={chunk_top_k}.jsonl"

    # ------------------------------------------------------------------
    # Log sinks
    # ------------------------------------------------------------------
    if context_max_tokens:
        log_path = out_subdir / f"chunk_{config_name}_maximum_input_length={maximum_input_length}_chunk_top_k={chunk_top_k}_context_max_tokens={context_max_tokens}.log"
    else:
        log_path = out_subdir / f"chunk_{config_name}_maximum_input_length={maximum_input_length}_chunk_top_k={chunk_top_k}.log"
    ...
    logger.info("chunk_size = {}, chunk_top_k = {}", chunk_size, chunk_top_k)
    logger.info("maximum_input_length = {}", maximum_input_length)

    with jsonlines.open(out_path, mode="w") as writer:
        for i, row in tqdm(
            enumerate(data_loader.iterate(), start=1), desc="chunk-attention"
        ):
            ...
            topk_chunks_full = [
                _short_hash(t) for t in _topk_attended_chunks_retriever(
                    model=model,
                    tokenizer=tokenizer,
                    retriever=retriever,
                    messages=row_full["messages"],
                    context_str=row_full["context"],
                    chunk_top_k=chunk_top_k,          # original_chunks=None (default)
                )
            ]
            ...
            # Intersection & output
            # --------------------------------------------------
            num_chunks = min(len(topk_chunks_full), len(topk_chunks_rag))
            topk_chunks_full = topk_chunks_full[: num_chunks]
            topk_chunks_rag = topk_chunks_rag[: num_chunks]

            intersection = len(set(topk_chunks_full) & set(topk_chunks_rag)) / num_chunks
            sum_intersections += intersection

            writer.write(
                {
                    "id": instance_id,
                    "topk_full_chunks": topk_chunks_full,
                    "topk_rag_chunks": topk_chunks_rag,
    ...
    logger.success(
        "Done - analysed {} instances | average chunk-overlap = {:.4f}",
        total_instances,
        avg_inter,
    )
...
if __name__ == "__main__":
    ...
    chunk_attn_analysis(cfg_path, dotlist_overrides)
