...
def main(args=None):
    ...
    parser.add_argument("--file", type=Path, required=True, help="JSONL result file")
    parser.add_argument("--output_dir", type=Path, default="data/evaluation", help="Where to write the prediction results.")
    ...
    for line in data:
        ...
        line["output"] = re.sub(r"\n+", " ", line['model_output'])
        line["output"] = line["output"].replace("<|im_end|>", "")

    # Remove all citations for all non-AutoAIS evaluation
    normalized_data = copy.deepcopy(data)
    for line in normalized_data:
        line["output"] = remove_citations(line['output'])
