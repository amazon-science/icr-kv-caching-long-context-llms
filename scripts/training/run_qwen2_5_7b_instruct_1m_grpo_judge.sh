...
export EXPERIMENT_NAME="qwen2_5_7b_instruct_1m_grpo_judge_v2"
export DATA_DIR="data"
export OUTPUT_DIR="data/outputs/${EXPERIMENT_NAME}"
export ROLLOUT_DIR="data/rollouts/${EXPERIMENT_NAME}"
...
mkdir -p $OUTPUT_DIR
mkdir -p $ROLLOUT_DIR ## Added to analyze CoT outputs.
mkdir -p $LOG_DIR
mkdir -p $WANDB_DIR

wandb login

hotpot_qa_train_path="$DATA_DIR/train/hotpot_qa/grpo/train_judge.parquet"
hotpot_qa_test_path="$DATA_DIR/train/hotpot_qa/grpo/test_judge.parquet"
WikiMultihopQA_train_path="$DATA_DIR/train/2WikiMultihopQA/grpo/train_judge.parquet"
WikiMultihopQA_test_path="$DATA_DIR/train/2WikiMultihopQA/grpo/test_judge.parquet"

train_files="['$hotpot_qa_train_path', '$WikiMultihopQA_train_path']"
test_files="['$hotpot_qa_test_path', '$WikiMultihopQA_test_path']"

reward_func_name="compute_score_batch"
reward_func_path="src/amzn_long_context_rag/training/judge_reward_functions.py"
